...
第二轮3frp8j8q已通过全部世界系窗口，但body_heading在首次进入yaw误差界后超调至0.751063rad，较目标0.6误差0.151063>原0.15；位置/速度仍合格，失败保留。定点转向的ready之后也采用同一最多12墙钟秒、连续1.5仿真秒稳定判定，再开始原2s驻留；同样逐1ms审计稳定段，不调整0.15rad界。

第四轮4q1lfex8运行时通过并落地，但原始逐1ms审计失败：PX4的world_step/world_reentry/body_step窗口起始分别有1/35/24 tick的高度误差超过0.5m，最大分别0.5001326/0.5190401/0.5143051m；其余窗口内无超界，AP全部主窗口满足原界。这是估计状态首次进入范围略早于物理真值，不能删去开头tick或放宽门槛。第五轮起，四个moving阶段在首次ready后固定保持新目标并检查状态新鲜2仿真秒，再开始原3s持续窗口；窗口开始即需满足全部原条件，任何违例立即失败。新增准备段不冒称全段跟踪合格，首次ready仍限20仿真秒，整轮900墙钟秒/180000tick上限不变；第四轮保留失败结论。
