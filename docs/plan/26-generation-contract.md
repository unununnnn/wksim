...
2026-09-10 最新实施：已从固定 e0 SLX 11.8 实际运行 MATLAB/Embedded Coder，完成新 C++ 生成、Linux 独立构建及现有核心 `Model` 的冷重建/重置验证，见[真实生成与生命周期报告](../2026-09-10-generated-e0-lifecycle.md)。本次 5 项许可证 test/checkout、9 个阶段、原厂/私有输入及新产物 SHA 均实际核验。厂商字节仍仅留本机；不把这项本地开发验证扩大为再分发许可。下文“尚无生成入口/checkout证据”的内容是历史检查点，不再作为当前缺口；G6 数值等价、默认模型提升与父票原依赖仍未因此完成。

主任务材料补充：[G6材料索引](../g6-material-index.md)已定位厂家模型工作流PDF第3页、e0说明书第1–3页及C++生成教程第6/9页，明确描述二次开发、代码生成和Windows/Linux等目标。这比下文历史文件名搜索提供了更强的本地开发用途依据，不能再概括为没有任何开发用途资料。完整路径、页序与SHA在索引中；再分发条款与工具实际checkout仍分别记录，未据索引宣称生成入口已交付或新模型已验证。
