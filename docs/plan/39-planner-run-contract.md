...
- **GridMap 三维膨胀 P1，静态修复已覆盖**：上游 `GridMap::cloudCallback` 把 z 方向膨胀步数写死为 2 个 voxel（XY 用 `obstacles_inflation/resolution`），`scanCallback` 不可达死代码还藏有 20 voxel z 膨胀。本仓库副本已修复：`cloudCallback` 与 `clearAndInflateLocalMap` 的膨胀步数统一由 `checkedInflationSteps` 取得、三轴相同（同一初始化/回调限幅语义，逐轴 ≤32 步，候选立方 ≤`65^3`）；`scanCallback` 死函数体已删除，回调保持 no-op、不启用 scan。`clearAndInflateLocalMap` 在 `toAddress(inf_pt)` 前逐轴 `isInMap(inf_pt)` 拒绝越界候选（扁平地址范围检查会把越界 z 别名到有效扁平地址）；`cloudCallback` 跳过非有限点，在每个膨胀候选转整数索引前检查有限值和地图位置界，再更新 local_bound；地图外邻近点仍可向界内膨胀，极端有限坐标不能经 `posToIndex` 溢出或扩大 `local_bound`。按 profile `obstacles_inflation=0.8`、`resolution=0.1`：8 步/轴、`17^3` 保守立方（碰撞半径 R=max(0.8, 0.35+0.30)=0.8m）。本切片已完成完整 `plan_env` C++17 构建及三端28项检查（Windows两项native跳过），原始构建/源码身份见 [构建收据](../../validation/39-gridmap-inflation-build-20260912/summary.json)；仍不声明真实点云、planner clearance 或回调耗时上界；真实三维碰撞包络仍必须由 #29 的物理几何/clearance 证据收口。
- **GridMap 参数回调 P1，静态修复已覆盖**：`GridMap::initMap` 仍使用局部变量读取启动期的 `uav_id`、尺寸和原点，但这些变量的地址不再保存到成员指针容器；`/uav1/prometheus/param_settings` 回调对 `param_name/param_value` 数量、前缀和解析结果做 fail-closed 校验。分辨率、尺寸、原点、`pose_type`、相机内参、深度比例、`skip_pixel`、概率、地面/虚拟顶等初始化期参数明确告警并忽略；局部更新范围按初始化 `map_size_` 逐轴限幅，`local_map_margin` 按 `map_voxel_num_` 和整数算术余量限幅，膨胀距离按初始化分辨率/地图尺寸限到最多 32 个 voxel（`65^3` 个候选），深度/射线 min/max 不能越过地图对角线或形成 `min>max`。深度过滤、射线范围、超时、布尔开关和 `frame_id` 只通过显式解析与域约束更新，不重建或改写地图边界、体素数量、buffer 或订阅结构。初始化/参数安全修改已随 adcf009 完成实际 plan_env 构建；运行期参数事件验证仍未完成。
