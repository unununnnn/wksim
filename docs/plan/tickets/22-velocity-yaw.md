...
状态：2026-09-09验收通过。原始0.5×正式联合场景完整通过，独立双栈补齐body及PX4角度组合；主代理原始审计两次字节一致，独立复核无影响收口的问题。详见[收口报告](../../2026-09-09-velocity-yaw-closure-report.md)。历史失败保留，持续1×/#20/G2及Full未完成。
...
- [x] AP所需原生入口或候选变更必须实际构建并匹配消息身份；不能只保留ROS字段。
- [x] 冻结激活轴、yaw与yaw-rate语义和控制门槛，支持与不支持的组合明确列出。
- [x] 两栈真实运行速度阶跃、停止保持和偏航工况，并以持续状态/真值验证。
- [x] 无效组合在受理前拒绝且没有控制副作用，已有位置任务回归通过。
