...
发送/接收端v2接线已完成主验：Sender显式accept_control启用上游Bool gate和私有Empty hold/cancel，同锁编码/写入、失败退役、cancel后抑制输出；Node按整批有序outcome折叠后再发布，close→open不复活旧激活，hold非误激活，cancel守原ACK边界再发公共Setup。释放截止改用ROS operation时钟10s，生产use_sim_time切换拒绝。OMP22i曾覆盖旧TCP测试且以wall计时，主会话恢复旧测试、新用例独立文件，并修正stub缺口/同批误发/等号边界。25项隔离ROS2生成消息/回调/真实TCP检查和131项纯sender/receiver/codec/pump检查通过；ACK仍是合成，未证明live ROS1订阅或FC停止。当前无主会话native运行，下一步真实发布/订阅及ControlNode验证；保持晚到起点规则。

Live ROS1 sender验证已完成：validation/ros1-sender-live-20260912-02，真实Bool/Bspline/Empty订阅进入sender进程，TCP实收gate/Bsp/hold/cancel序号1–4，原始EGO时间2601000000ns、knots/点/ID保真，cancel后0.3s观察无额外bytes，sender/roscore退出0。01仅因夹具list/tuple比较错误失败，原记录与源保留。该实验无ROS2 Node/ControlNode/FC，不是停止确认。下一步联合发送/接收与实际ControlNode/FC证明。当前无主会话native运行。

当前Control新候选BeqOco已实际构建并live sealer通过：/root/wksim-joint-control-BeqOco/build.json，SHA0b6d6de16bba35acd964828c3d4f220abe1a36b1d5d5533686e76944f1ee3f07。先前avGyUP安装smoke揭示PlannerTransportNode覆盖rclpy._subscriptions导致/clock虽被发现却不执行；改为_planner_subscriptions后，26项ROS检查及仓库外安装态真实/clock→TCP cancel→SetupRequest装配通过。Setup被capture不实际发布、release_confirmed=false，不是FC证明；未更新最终飞行准入pins。证据validation/control-install-v2-20260912含失败原件。6eac431推送仍TLS失败，当前后续提交也需网络恢复后推送，不禁用证书验证。

真实release验证准备：Ubuntu原生Git创建/root/wksim-release-acceptance-fe3，分支codex/planner-release-validation，基线fe3ced7；Windows侧另有wksim-release-acceptance只作查阅，不用于WSL执行（C: gitdir路径不兼容）。曾尝试relative-worktrees触发旧WSL Git不支持，已回退并删除该扩展，主仓库WSL rev-parse已验证正常；不要重启该路径尝试。message verifier现允许同一Git common-dir工作区验证原owned validation证据，仍核对顶层路径/无symlink/原manifest及所有SHA；Windows4项与Linux实际Git测试通过，Linux快照check_control(BeqOco)已成功，无须重建相同消息。原工作区他方runtime/runner等改动仍未触碰。下一步在此Linux分支实现明确标记的真实release场次，不把它当#83 nominal PV；先明确移动中的writer静默、SessionState双高水位移交、Node cancel→BRAKE双相确认、原停止阈值与正常LAND。Claude18-AG只读方案已完成，但其endpoint-held时机不代表moving；必须自行核对并选择确实移动的触发点。当前无主会话native运行。
