...
def _schedstat_valid(row):
    ...
    return isinstance(schedstat, dict) and schedstat.get("runqueue_counters_valid") is True
...
def _delta_pair(delta, unavailable, negative, name, before_value, after_value,
    ...
    if before_value is None or after_value is None:
        ...
        return
    if not _is_plain_int(before_value, 0) or not _is_plain_int(after_value, 0):
        delta[name] = None
        unavailable[name] = "invalid_counter_value"
        return
