...
def ap_observation_requests(root, result, messages, phases):
    from pymavlink.dialects.v20 import common
    rows = list(lines(root/'hex-native.jsonl'))
    requests = [r for r in rows if r['kind']=='observation_interval_tx']
    acknowledgements = [r for r in rows if r['kind']=='observation_interval_ack']
    require([r['message_id'] for r in requests]==[87,85,245]
            and [r['message_id'] for r in acknowledgements]==[87,85,245], 'Missing/reordered AP observation requests/ACKs')
    ready = phases['public_control_ready']['observed_monotonic_s']
    require(ready < phases['arming_completed']['observed_monotonic_s'], 'AP observation setup not before arm')
    previous = -math.inf
    for request, ack in zip(requests,acknowledgements):
        raw = bytes.fromhex(request['datagram_hex'])
        decoded = common.MAVLink(None).parse_buffer(raw)
        require(decoded and len(decoded)==1 and bytes(decoded[0].get_msgbuf())==raw, 'Invalid observation request frame')
        msg = decoded[0]
        require(msg.get_type()=='COMMAND_LONG' and msg.get_srcSystem()==245 and msg.get_srcComponent()==190
                and msg.command==511 and msg.target_system==241 and msg.target_component==1 and msg.confirmation==0
                and msg.param1==request['message_id'] and msg.param2==request['interval_us']==100000
                and [msg.param3,msg.param4,msg.param5,msg.param6,msg.param7]==[0.]*5,
                'Observation socket carried nonallowlisted request')
        require(previous < request['monotonic'] <= ack['monotonic'] < ready
                and ack['monotonic']-request['monotonic'] < result['protocol']['parameter_read_timeout_s'],
                'Observation request/ACK deadline or ordering differs')
        matches = [r for r in messages if request['monotonic']<=r['monotonic']<=ack['monotonic']
                   and r['packet_hex']==ack['packet_hex']]
        require(len(matches)==1, 'Observation ACK absent from original native datagrams')
        r = matches[0]; m = r['message']
        require(r['source_system']==241 and r['source_component']==1 and r['peer']==request['peer']
                and m['mavpackettype']=='COMMAND_ACK' and m['command']==511
                and m['target_system']==245 and m['target_component']==190 and m['result']==ack['result']==0,
                'Observation ACK identity/result differs')
        previous = ack['monotonic']
    return dict(message_ids=[87,85,245], interval_us=100000, raw_ack_count=3,
                decoder_sha256=digest(common.__file__),
                association='Sequential single outstanding SET_MESSAGE_INTERVAL; COMMAND_ACK has no message-id echo')
...
def ap_goal(target, home, origin):
    """Pinned SITL Location/Guided/float log representations, not a physical fit."""
    require(target['coordinate_frame'] == 6 and target['type_mask'] == 0x9f8
            and target['header']['frame_id'] == 'map' and near(target['yaw'], 0.), 'AP global DDS axes differ')
    lat, lon = int(target['latitude']*1e7), int(target['longitude']*1e7)
    alt = int(float32(target['altitude'])*100.)
    # AP DDS Location(int32,int32,int32) -> WPNav double NE, float altitude.
    origin_alt = int(origin['altitude']/10)
    mid = int((lat+origin['latitude'])/2)
    north = (lat-origin['latitude'])*1.1131884502145034*.01
    delta_lon = (lon-origin['longitude']+1800000000)%3600000000-1800000000
    east = delta_lon*1.1131884502145034*max(math.cos(mid*(1e-7*(math.pi/180.))), .01)*.01
    down = -float32((home['home_altitude_cm']+alt-origin_alt)*.01)
    # get_wp: from_ekf_offset_NED_m(Vector3d), then offset_latlng. Its inverse
    # scaling constant is float, although the position/ftype arithmetic is double.
    inverse = float32(89.83204953368922)
    dlat = int((north*100.*.01)*inverse)
    dlng = int((east*100.*.01)*inverse / max(math.cos((origin['latitude']+int(dlat/2))*(1e-7*(math.pi/180.))), .01))
    global_lat = origin['latitude']+dlat
    global_lon = (origin['longitude']+dlng+1800000000)%3600000000-1800000000
    absolute_cm = int(-down*100.)+origin_alt
    return dict(log=[float32(x) for x in (lat,lon,alt)], local=[float32(x) for x in (north,east,down)],
                global_target=[global_lat,global_lon,float32(float32(absolute_cm)*float32(.01))],
                native_stamp_s=stamp(target), input_global=[lat,lon,alt])


def ap_target_receipts(data, messages, phases, guided):
    start, end = [phases[p]['observed_monotonic_s'] for p in ('waypoint_accepted','land_accepted')]
    boot_end = phases['land_accepted']['native_boot_s']
    all_targets = data['/ap/cmd_gps_pose']
    before = [(r,t) for r,t in all_targets if r['monotonic'] < start]
    targets = [(r,t) for r,t in all_targets if start <= r['monotonic'] < end]
    require(before and targets, 'Missing AP preceding/waypoint DDS target history')
    goals = []
    seen_new_dds = False
    for index, (raw, target) in enumerate([before[-1], *targets]):
        homes = [v for _,v in data['/ap/wksim/local_state_v1'] if v['time_boot_us'] == round(stamp(target)*1e6)]
        origins = [m['message'] for m in messages if m['message']['mavpackettype']=='GPS_GLOBAL_ORIGIN'
                   and m['monotonic'] <= raw['monotonic']]
        require(homes and origins, 'AP target lacks original Home/origin binding')
        home, origin = homes[-1], origins[-1]
        goal = ap_goal(target,home,origin)
        ...
        is_new = index > 0 and target['latitude']==lat/1e7 and target['longitude']==lon/1e7 and near(target['altitude'],3.)
        if index > 0 and not is_new:
            require(not seen_new_dds and goal['input_global']==goals[0][1]['input_global'], 'AP DDS global-home binding differs')
        seen_new_dds |= is_new
        goals.append((is_new,goal))
    require(seen_new_dds, 'No requested AP waypoint DDS target')
    def associate(rows, representation, read_value, prior_until=None):
        require(rows, 'No AP '+representation+' target receipt')
        seen_new = False; new_count = prior_count = 0; first_new = None
        for at, m in rows:
            matches = [(new,g) for new,g in goals if g['native_stamp_s'] <= at < boot_end
                       and (near(read_value(m),g[representation]) if representation=='local'
                            else read_value(m)==g[representation])]
            current = any(new for new,_ in matches)
            require(current or (not seen_new and (prior_until is None or at <= prior_until)
                                and any(not new for new,_ in matches)),
                    'Unexplained AP '+representation+' target or previous goal after new receipt')
            seen_new |= current
            if current and first_new is None:
                first_new = at
            new_count += int(current); prior_count += int(not current)
        require(seen_new, 'No new AP waypoint '+representation+' receipt before land')
        return new_count, prior_count, first_new
    for g in guided:
        require(g['Type']==2 and g['Terrain']==0 and all(g[k]==0 for k in ('vX','vY','vZ','aX','aY','aZ')),
                'AP GUIP non-position/terrain/velocity/acceleration target')
    new, prior, first_new = associate([(g['TimeUS']/1e6,g) for g in guided], 'log', lambda g:[g['pX'],g['pY'],g['pZ']])
    receipt_counts = {}
    for kind, representation, fields, frame, mask in (
            ('POSITION_TARGET_GLOBAL_INT','global_target',('lat_int','lon_int','alt'),0,0xfdf8),
            ('POSITION_TARGET_LOCAL_NED','local',('x','y','z'),1,0xdf8)):
        selected = [m['message'] for m in messages if m['message']['mavpackettype']==kind and start<=m['monotonic']<end]
        require(all(m['coordinate_frame']==frame and m['type_mask']==mask for m in selected), 'AP native target frame/axes differ')
        associate([(m['time_boot_ms']/1000,m) for m in selected], representation, lambda m:[m[k] for k in fields], first_new)
        receipt_counts[representation] = len(selected)
    return dict(dds_targets=len(targets), new_goal_guip_targets=new, preceding_guip_targets=prior,
                global_receipts=receipt_counts['global_target'], local_receipts=receipt_counts['local'],
                association='Source timestamps and exact/float32 values; no per-request native identity or exact first acceptance tick')
...
def native_delivery(root, result, data, messages, phases):
    ...
    observation = ap_observation_requests(root,result,messages,phases)
    from pymavlink import DFReader
    # Pinned SITL: Storage.cpp uses eeprom.bin; AP_Logger_File generates
    # HAL_BOARD_LOG_DIRECTORY (logs) / %08u.BIN. Both remain hash-checked above.
    dataflash = [name for name in logs if re.fullmatch(r'logs/[0-9]{8}\.BIN',name)]
    require(all(name=='eeprom.bin' or name in dataflash for name in logs), 'Unknown AP binary artifact path')
    require(len(dataflash) == 1, 'Expected one AP DataFlash logs/NNNNNNNN.BIN')
    files = [root/dataflash[0]]
    ...
    receipts = ap_target_receipts(data,messages,phases,guided)
    ...
    return dict(**receipts, observation_requests=observation, bin_guided_targets=len(guided), native_logs=logs,
