...
from tools.hex_candidate import native_parameters, PLAN_IDENTITY, BUILD_SHA
...
def identity(root, result):
    ...
    require(digest(root/'protocol.json') == PROTOCOL_SHA == result['protocol_sha256']
            == digest(REPO/PROTOCOL_PATH), 'Frozen physical budget identity changed')
    ...
    require(ids['hex_build_sha256'] == BUILD_SHA and ids['hex_library_sha256'] == LIBRARY_SHA256
            and ids['plan_identity'] == PLAN_IDENTITY and ids['hex_config']['model_identity'] == MODEL_IDENTITY,
    ...
    for name in ('tools/hex_physics.py', 'tools/build_hex_model_candidate.py', 'tools/hex_launch_plan.py',
        ...
        require(digest(REPO/name) == result['source_sha256'][name], 'Shared decoding/identity recipe changed: '+name)
    ...
    expected_identity = dict(model_identity=MODEL_IDENTITY, plan_identity=PLAN_IDENTITY, model_profile='hex_x',
    ...
    if result['stack'] == 'px4':
        ...
        require((root/'hex.parm').read_text() == launch_plan()['ap_parameter_file']+'LOG_DISARMED 1\n'
...
def reset_links(result, parent, parent_sha):
    ...
    require(result['admission']['configuration_identity'] == parent['admission']['configuration_identity'] == link['configuration_identity']
            and result['protocol_sha256'] == parent['protocol_sha256'] == PROTOCOL_SHA, 'Cold reset changed configuration/budget')
