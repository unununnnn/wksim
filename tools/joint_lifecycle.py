...
class JointLifecycle:
    ...
    def receive(self, uid, message):
        ...
        now = time.monotonic()
        if (session is None or value.get('version') != 1 or value.get('run_id') != self.run_id
                or value.get('scene_epoch') != self.clock.epoch or value.get('uav_id') != uid
            ...
            raise ValueError('Invalid scene control acknowledgement')
        self.acks[uid] = value
    ...
    def periodic(self, force=False):
        ...
        if force or now >= self.next_publish:
            self.sequence += 1
            value = dict(version=1, run_id=self.run_id, scene_epoch=self.clock.epoch,
                ...
                issued_monotonic_s=now, lease_seconds=LEASE_SECONDS)
