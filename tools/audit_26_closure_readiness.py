...
def _check_source_actual(item, basename, root, report, label, identities):
    ...
    if basename == "model.cpp":
        ...
        # Build receipts retain the absolute path of the machine that produced
        # them.  In a clean checkout on another host, bind the exact canonical
        # wrapper suffix to this checkout and verify its committed HEAD blob.
        # A file that merely shares the model.cpp basename is rejected above.
        source = _secure_file(root, repo_rel, report, f"{label}.source_path")
        if source is None:
            return False
        committed = _check_committed_identity(
            root,
...
def _repo_relative_from_source(value, root):
    ...
    try:
        ...
        # Historical receipts preserve their producing checkout's absolute
        # path.  Only a complete approved wrapper suffix is portable; a bare
        # same-basename path remains external and is rejected by its caller.
        if WINDOWS_ABSOLUTE_RE.match(value) or value.startswith("/"):
            normalized = value.replace("\\", "/").rstrip("/")
            for relative in ALLOWED_REPO_WRAPPERS:
                if normalized.endswith("/" + relative):
                    return relative
        return None


def _private_generated_relative_from_source(value, basename):
    """Return the exact pinned private path encoded by a historical receipt."""
    if basename not in REQUIRED_GENERATED:
        return None
    expected = f"{PINNED_GENERATED_ROOT}Exp1_MinModelTemp_ert_rtw/{basename}"
    normalized = value.replace("\\", "/").rstrip("/")
    if not (WINDOWS_ABSOLUTE_RE.match(value) or value.startswith("/")):
        return None
    ...
    try:
        ...
        for basename, item in staged.items():
            ...
            if repo_rel is not None:
                ...
                basename in REQUIRED_GENERATED
                and _private_generated_relative_from_source(item["source_path"], basename) is None
            ):
