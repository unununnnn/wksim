"""Bounded independent GNSS fault experiment; completion still requires raw audit."""
import argparse
import hashlib
...
def launch_plan(config, directory, library, scene_epoch):
    ...
    plan['physics'] = [sys.executable,'-B',str(REPO/'tools/gnss_physics.py'),
        '--stack',config['stack'],'--library',str(library),'--port','19002' if ap else '4581',
        '--trace',str(directory/'truth.jsonl'),'--profile',str(directory/'gnss-profile.json'),
        '--run-id',config['run_id'],'--scene-epoch',scene_epoch]
    if ap:
        files=plan['fc'][plan['fc'].index('--defaults')+1].split(',')
        files.insert(-1,str(directory/'gnss.parm'))
        plan['fc'][plan['fc'].index('--defaults')+1]=','.join(files)
        plan['fc_environment'].update(WKSIM_RUN=config['run_id'],WKSIM_EPOCH=scene_epoch,WKSIM_GNSS_TRACE=str(directory/'gnss-native.tsv'))
    return plan


def run(admission, output, scene_epoch):
    check_isolation()
    ...
    if output.exists() or output.parent != Path('/root') or not output.name.startswith('wksim-gnss-flight-'):
        raise ValueError('Use a fresh /root/wksim-gnss-flight-* output directory')
    ...
    result = dict(status='failed', experimental=True, production_admitted=False,
        ...
        scene_epoch=scene_epoch,scope='Independent GNSS fault; native failsafe and explicit new task recovery')
    ...
    source_names = ['tools/run_gnss_flight.py','tools/run-gnss-flight.sh','tools/gnss_physics.py',
        'tools/gnss_candidate.py','tools/rc_candidate.py','tools/joint_control_candidate.py','tools/audit_gnss_flight.py',
        'tools/pid_physics.py','Simulator/wksim_runtime/gnss_task.py','Simulator/wksim_runtime/gnss-flight-v3.json',
        'Simulator/wksim_runtime/rc_task.py','Simulator/wksim_runtime/efficiency_task.py',
    ...
    with Reservation(resource) as reservation:
        ...
        try:
            ...
            shutil.copy2(REPO/'Simulator/wksim_runtime/gnss-flight-v3.json',directory/'gnss-profile.json')
            ...
            if config['stack']=='arducopter':
                (directory/'gnss.parm').write_text('SIM_GPS1_ENABLE 1\nSIM_GPS1_TYPE 1\nSIM_GPS1_LAG_MS 100\nSIM_GPS1_HZ 5\nSIM_GPS1_BYTELOS 0\nSIM_GPS2_ENABLE 0\nSIM_GPS2_TYPE 0\nGPS1_TYPE 2\nGPS_AUTO_CONFIG 0\nGPS_DRV_OPTIONS 4\n')
            ...
            ros=rclpy; ros.init()
            task=GNSSTask(directory,health,phase,config['stack'],run_id=config['run_id'],protocol='session_v1',scene_epoch=scene_epoch)
            for name,child,_ in children:
...
def main():
    ...
    if not args.prepared:
        ...
        script+='exec '+shlex.join([sys.executable,'-B',str(Path(__file__).resolve()),'--prepared',
            ...
            '--scene-epoch',args.scene_epoch,'--control-manifest',args.control_manifest,'--control-sha256',args.control_sha256]+(['--preflight'] if args.preflight else []))+'\n'
        os.execvp('bash',['bash','-c',script])
    from gnss_candidate import admit
    from Simulator.wksim_runtime.gnss_task import load_profile
    admission=admit(args.stack,args.run_id,args.control_manifest,args.control_sha256,load_profile())
    if args.preflight:
        print(json.dumps(admission,indent=2)); return 0 if admission['ok'] else 2
    result=run(admission,args.output_root,args.scene_epoch)
