...
def audit(root,verify_current_sources=False):
    ...
    require(digest(root/'control-build.json')==r['manifest_sha256']['control'],'Control manifest changed')
    control=r['control_candidate']
    require(json.loads((root/'control-build.json').read_text())==control,'Control build record differs')
    for directory in (Path(control['package']),Path(control['root'])/'src/prometheus_control/prometheus_control'):
        actual={p.relative_to(directory).as_posix():digest(p) for p in directory.rglob('*.py') if '__pycache__' not in p.parts}
        require(actual==control['python_sha256'],'Executed/staged control candidate changed')
    ...
    for stack in ('arducopter','px4'):
        ...
        for tick,row in enumerate(lines(root/(stack+'-truth.jsonl')),1):
            ...
            data.append((state[6:12],commands))
            if stack=='arducopter':
        ...
        require(physical[stack]['max_height_m']>=2.5 and abs(physical[stack]['final_height_m'])<.3
                and physical[stack]['min_waypoint_error_m']<=.5,'Independent truth failed')
    ...
    for tick,c in enumerate(lines(root/'clock.jsonl')):
        ...
        clock_count+=1
    repetitions=r.get('paused_clock_republications',0)
    require(type(repetitions) is int and repetitions>=0,'Invalid clock retransmission count')
    if repetitions:
        require(r.get('scene_lifecycle_requested') and r['scene_lifecycle']['status']=='pass',
                'Unexpected paused clock publication outside a lifecycle run')
        repeated=0
        for entry in lines(root/'scene-lifecycle.jsonl'):
            if entry['kind']=='paused_clock':
                repeated+=1
                require(entry['epoch']==epoch and entry['phase']=='paused' and entry['tick']%4==0
    ...
    artifacts={p.relative_to(root).as_posix():digest(p) for p in sorted(root.rglob('*')) if p.is_file() and p.name!='audit.json'}
    return dict(status='pass',scope='continuous public joint Task flight only; no pause/recovery or collision acceptance',
                ...
                total_ticks=total,shared_epoch=epoch,tasks=task_reports,physical=physical,
                ...
                paused_clock_republications=repetitions,
