...
def audit(directory):
    ...
    require(r['result']['status']=='stopped' and all(not e['remaining_group_members'] for e in r['result']['epochs']),
    ...
    require(config['quaternion_xyzw']==[0,0,0,1],'This audit covers the declared identity camera mount')
    packets={}
    ...
    for frame in r['frames']:
        ...
        require(data['schema']=='wksim.rgb.v1' and data['run_id']==r['config']['run_id'] and
        ...
        checked.append(dict(step=step,frame_id=data['frame_id'],metadata_sha256=sha(metadata),image_sha256=sha(image),
                            position_error_cm=position,quaternion_l2=quaternion,rgb_mean=stat.mean,rgb_extrema=extrema))
    require(len(checked)>=15 and all(b['step']>a['step'] for a,b in zip(checked,checked[1:])),'Missing or replayed consumed frames')
    ...
    return dict(status='pass',report_sha256=sha(directory/'report.json'),build_binary_sha256=build['binary_sha256'],
                ...
                limitations=['Ground only, no flight or geometric/pixel calibration acceptance.',
