"""Bounded independent #34 candidate run; no production profile or default changes."""
import argparse
import hashlib
...
def run(admission, output):
    ...
    if output.exists() or output.parent != Path('/root') or not output.name.startswith('wksim-attitude-flight-'):
        raise ValueError('Use a fresh /root/wksim-attitude-flight-* output directory')
    ...
    result = dict(status='failed', experimental=True, production_admitted=False,
        ...
        scope='Independent attitude/thrust candidate; speedup requested 1, no joint rate claim')
    ...
    source_names = ['tools/run_attitude_flight.py','tools/run-attitude-flight.sh','tools/attitude_physics.py',
    ...
    with Reservation(resource) as reservation:
        ...
        try:
            ...
            ros=rclpy; ros.init()
            task=AttitudeTask(directory,health,phase,config['stack'],run_id=config['run_id'],protocol='session_v1')
            for name,child,_ in children:
...
def main():
    ...
    result=run(admission,args.output_root)
