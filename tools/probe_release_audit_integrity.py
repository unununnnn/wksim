...
Coverage semantics
------------------
``--scenario`` may run a subset, which is useful while debugging one case: the
``verdict`` then describes only the executed subset and ``selected_matrix_pass``
mirrors it.  That is *not* evidence that the whole matrix holds.  The report
therefore also carries an explicit ``coverage`` block and a top-level
``complete_matrix_pass`` flag, which is true only when the positive control was
accepted, all four required negatives were really rejected, and the raw
baseline is unchanged.  A partial or blocked run can be ``selected_matrix_pass``
while ``complete_matrix_pass`` stays false.
...
* Large files are hardlinked into the copy to avoid duplicating evidence;
  ...
  copy first.
* Every resolved audit input must be a real file inside the raw root.  An input
  that is itself a symlink, or that is reached through a symlinked/junction
  directory escaping the raw root, is rejected *before* any output directory or
  isolated copy is created.  This keeps the hashed baseline object and the
  copied object the same file.
...
def _reject_symlinked_input(raw_root: Path, rel: str) -> Path:
    """Reject an audit input that is, or is reached through, a link.

    ``Path.is_file`` follows links, so without this a baseline could hand the
    probe a symlink (or a path under a symlinked/junction directory) pointing
    outside the raw root; the copy would then duplicate an object the baseline
    hash never described.  ``raw_root`` must already be resolved.

    The containment check is what catches directory junctions, which are not
    always reported by ``os.path.islink`` on Windows.
    """
    cursor = raw_root
    for part in Path(rel).parts:
        ...
        if os.path.islink(cursor):
            raise ProbeError(
                f"audit input {rel} traverses a symlink or junction: {cursor}")
    path = raw_root / rel
    real = Path(os.path.realpath(path))
    if not is_within(real, raw_root):
        raise ProbeError(
            f"audit input {rel} resolves outside the raw root: {real}")
    return path
...
def resolve_audit_inputs(raw_root: Path) -> dict:
    ...
    Every candidate is vetted by :func:`_reject_symlinked_input` first, so a
    linked or escaping input raises instead of being silently followed.
    """
    files = {}
    missing = []
    for rel in AUDIT_INPUTS:
        rel = refuse_forbidden(rel)
        path = _reject_symlinked_input(raw_root, rel)
        if path.is_file():
            files[rel] = path
        else:
            missing.append(rel)
...
def probe_tool_identity():
    """SHA-256 of this tool's own bytes.

    Recorded separately from ``auditor_sha256`` so a report always says which
    probe produced it as well as which auditor it exercised.
    """
    try:
...
def run_probe(args) -> dict:
    ...
    # Vet every input before creating any output directory or isolated copy, so
    # a rejected symlink/junction leaves nothing half-built behind.
    inputs = resolve_audit_inputs(raw_root)
    if RESULT_REL in inputs["missing"]:
    ...
    probe_tool_file, probe_tool_sha256 = probe_tool_identity()
    log(f"probe sha    : {probe_tool_sha256}")
    ...
    selected_pass = verdict == "pass"
    coverage = summarize_coverage(scenarios, unchanged, selected_pass)
    log(f"\ncoverage     : required={len(coverage['required'])} "
        f"executed={len(coverage['executed'])} missing={coverage['missing']}")
    log(f"matrix       : selected_matrix_pass={selected_pass} "
        f"complete_matrix_pass={coverage['complete_matrix_pass']}")

    notes = limitations(scenarios, codec, codec_error)
    if not coverage["complete_matrix_pass"]:
        if coverage["missing"]:
            notes.append(
                "partial matrix: required scenarios not executed ("
                + ", ".join(coverage["missing"])
                + "); a pass verdict covers only the executed subset and is NOT "
                "a complete one-positive/four-negative result")
        elif not coverage["positive_accepted"]:
            notes.append("complete matrix not achieved: the positive control was "
                         "not accepted")
        elif coverage["negatives_not_rejected"]:
            notes.append("complete matrix not achieved: required negatives not "
                         "rejected: "
                         + ", ".join(coverage["negatives_not_rejected"]))
        elif not unchanged:
            notes.append("complete matrix not achieved: the raw baseline changed")
        elif not coverage["verdict_pass"]:
            extra = sorted(set(coverage["blocked_or_error"])
                           | set(coverage["optional_executed"]))
            detail = ("; affected: " + ", ".join(extra)) if extra else ""
            notes.append(
                "complete matrix not achieved: the run verdict is not pass, so an "
                "explicitly requested optional scenario was blocked, errored, or "
                "not rejected" + detail)
    report = {
        ...
        "probe_tool_file": str(probe_tool_file) if probe_tool_file else None,
        "probe_tool_sha256": probe_tool_sha256,
        ...
        "audit_inputs": {
            ...
            "derived_retained_sources": sorted(set(inputs["derived"])),
        ...
        "coverage": coverage,
        "selected_matrix_pass": selected_pass,
        "complete_matrix_pass": coverage["complete_matrix_pass"],
        "scenarios": scenarios,
        "verdict": verdict,
...
def summarize_coverage(scenarios, baseline_unchanged, verdict_pass):
    """Describe which parts of the required matrix actually ran.

    ``complete_matrix_pass`` is deliberately stricter than ``verdict``: the
    required matrix must hold (positive control accepted, all four required
    negatives genuinely rejected, raw baseline unchanged) *and* the run's own
    verdict must be a pass.  That second condition is what stops an explicitly
    requested optional scenario which is blocked, errored, or not rejected from
    being masked by an otherwise complete required matrix.  Merely raising on
    everything, or only running a selected subset, cannot set it.
    """
    required = list(REQUIRED_SCENARIOS)
    optional = list(OPTIONAL_SCENARIOS)
    by_name = {entry["name"]: entry for entry in scenarios}

    executed = [name for name in required if name in by_name]
    missing = [name for name in required if name not in by_name]
    optional_executed = [name for name in optional if name in by_name]
    rejected = [name for name in executed
                if by_name[name]["outcome"] == "rejected"]
    not_rejected = [name for name in executed
                    if by_name[name]["outcome"] != "rejected"]
    blocked_or_error = sorted(
        name for name, entry in by_name.items()
        if entry["outcome"] in ("env_missing", "error"))

    positive = by_name.get("positive_unmodified")
    positive_accepted = positive is not None and positive["outcome"] == "accepted"

    required_matrix = (positive_accepted
                       and not missing
                       and len(rejected) == len(required)
                       and bool(baseline_unchanged))
    complete = bool(required_matrix) and bool(verdict_pass)
    return {
        "required": required,
        "executed": executed,
        "missing": missing,
        "optional_executed": optional_executed,
        "positive_executed": positive is not None,
        "positive_accepted": positive_accepted,
        "negatives_rejected": rejected,
        "negatives_not_rejected": not_rejected,
        "blocked_or_error": blocked_or_error,
        "baseline_unchanged": bool(baseline_unchanged),
        "required_matrix_pass": bool(required_matrix),
        "verdict_pass": bool(verdict_pass),
        "complete_matrix_pass": complete,
    }
...
def build_parser():
    ...
    parser.add_argument("--scenario", action="append", default=None,
                        ...
                        help="run only this scenario (repeatable); default all required. "
                             "A subset run can report a pass verdict for the executed "
                             "subset while complete_matrix_pass stays false")
