"""Bounded independent RC candidate experiment; completion still requires raw audit."""
import argparse
import hashlib
...
def launch_plan(config, directory, library):
    ...
    plan['control'] += ['-p', 'rc_boot_id:='+Path('/proc/sys/kernel/random/boot_id').read_text().strip().replace('-', '')]
    ap = config['stack']=='arducopter'
    if ap:
        plan['fc'][plan['fc'].index('--speedup')+1] = '1'
    else:
        plan['fc_environment']['PX4_SIM_SPEED_FACTOR'] = '1'
    plan['physics'] = [sys.executable, '-B', str(REPO/'tools/attitude_physics.py'),
        '--stack', config['stack'], '--library', str(library),
        '--port', '19002' if ap else '4581', '--trace', str(directory/'truth.jsonl'),
        '--raw', str(directory/'physics-1ms.jsonl')]
    return plan


def run(admission, output, scenario):
    check_isolation()
    ...
    if output.exists() or output.parent != Path('/root') or not output.name.startswith('wksim-rc-flight-'):
        raise ValueError('Use a fresh /root/wksim-rc-flight-* output directory')
    ...
    result = dict(status='failed', experimental=True, production_admitted=False,
        ...
        scenario=scenario, scope='Independent RC candidate; requested speedup 1, no joint rate claim')
    ...
    source_names = ['tools/run_rc_flight.py', 'tools/run-rc-flight.sh', 'tools/rc_candidate.py',
        'tools/joint_control_candidate.py', 'tools/attitude_physics.py', 'tools/audit_rc_flight.py',
        'Simulator/wksim_runtime/rc_task.py', 'Simulator/wksim_control/rc_input.py',
        ...
        'Simulator/wksim_runtime/evidence.py', 'Simulator/wksim_runtime/joint_profile.py',
    ...
    with Reservation(resource) as reservation:
        ...
        try:
            ...
            import rclpy
            from Simulator.wksim_runtime.rc_task import RCTask
            from Simulator.wksim_runtime.task import grounded
            ros=rclpy; ros.init()
            task=RCTask(directory,health,phase,config['stack'],run_id=config['run_id'],protocol='session_v1',
                scenario=scenario, boot_id=Path('/proc/sys/kernel/random/boot_id').read_text().strip().replace('-', ''),
                stream_id=__import__('uuid').uuid4().hex, truth_path=directory/'truth.jsonl')
            for name,child,_ in children:
...
def main():
    ...
    parser.add_argument('--scenario', required=True, choices=('movement','recenter','yaw','stream-stall','mode-out','new-takeover'))
    ...
    if not args.prepared and not args.preflight:
        from tools.joint_control_candidate import check
        ...
        script+='exec '+shlex.join([sys.executable,'-B',str(Path(__file__).resolve()),'--prepared',
            ...
            '--scenario',args.scenario,'--control-manifest',args.control_manifest,'--control-sha256',args.control_sha256])+'\n'
    ...
    result=run(admission,args.output_root,args.scenario)
