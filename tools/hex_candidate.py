...
def admit(stack, run_id):
    ...
    try:
        ...
        plan = launch_plan()
        require(plan['plan_identity'] == PLAN_IDENTITY, 'Frozen six-channel plan differs')
        ...
        identity = dict(model_identity=MODEL_IDENTITY, plan_identity=PLAN_IDENTITY, model_profile='hex_x',
        ...
        result.update(ok=True, config=runtime, library=str(library), setup_files=profile['setup_files'],
            ...
            identities=dict(baseline=fixed, hex_build_sha256=BUILD_SHA, hex_library_sha256=LIBRARY_SHA256,
                hex_config=config, plan_identity=PLAN_IDENTITY, parameters_xml_sha256=digest(xml),
