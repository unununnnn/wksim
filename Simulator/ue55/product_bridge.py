...
def display_packet(envelope, run_id, vehicle_id, roundtrip_s, windows_wall):
    ...
    if packet is None:
        return None
    validate(packet, run_id, vehicle_id, now=relay_wall)
...
def bridge(repo, path, run_id, vehicle_id=1, port=19060, readback=None):
        ...
        try:
            while True:
                ...
                try:
                    ...
                    mapped = display_packet(envelope, run_id, vehicle_id,
                                            time.monotonic()-requested_at, time.time())
                except (ValueError, TypeError, OverflowError, RecursionError):
                    pass
                if mapped and latest.accept(json.dumps(mapped), now=envelope['relay_wall_time_s']):
                    try:
                        udp.sendto(json.dumps(latest.packet, separators=(",", ":")).encode(), ("127.0.0.1", port))
                        if evidence:
                            pending[latest.packet["sequence"]] = latest.packet
                            if len(pending) > 64:
                ...
                for _ in range(64):
                    try:
                        ...
                        break
                    if not evidence or sender != ("127.0.0.1", port) or len(ack_raw) > MAX_PACKET:
                        continue
                    try:
...
if __name__ == "__main__":
    ...
    bridge(args.wsl_repo, args.state_socket, args.run_id, args.vehicle_id, args.port, args.readback)
