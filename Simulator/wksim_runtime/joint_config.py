...
from .joint_aruco_profile import PROFILE as ARUCO_PROFILE, TASK as ARUCO_TASK, validate_experiment
...
def validate_joint_config(data):
    ...
    if not isinstance(data,dict) or set(data)-required-{'task','requested_rate','task_dwell_seconds','display_socket','aruco_experiment'} or required-set(data):
    ...
    if task not in ('public_position','public_velocity_yaw',*FIXED_TASKS,ARUCO_TASK):
    ...
    if task == ARUCO_TASK:
        if data['runtime_profile'] != ARUCO_PROFILE:
            raise ConfigError('ArUco task requires the explicit experimental profile')
        try: validate_experiment(data.get('aruco_experiment'))
        except ValueError as error: raise ConfigError(str(error)) from error
    elif 'aruco_experiment' in data or data['runtime_profile'] == ARUCO_PROFILE:
        raise ConfigError('ArUco experiment resources belong only to the ArUco experimental task')
    ...
    if task in (*FIXED_TASKS,ARUCO_TASK) and (rate != .5 or ({'hold':5,'waypoint':2}|dwell) != {'hold':5,'waypoint':2}):
