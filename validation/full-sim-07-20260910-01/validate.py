...
def main():
    contract = json.loads((ROOT / 'docs/plan/full-contracts/sim-07.json').read_text(encoding="utf-8"))
    ...
    ledger_row = next(row for row in ledger["entries"] if row.get("id") == 'SIM-07')
    followup_row = next(row for row in followups["entries"] if row.get("id") == 'SIM-07')
    assert contract["issue"] == 125
    assert contract["source"]["line"] == 19
    assert contract["full_complete"] is False
    assert ledger_row["gap"] is True
    assert followup_row["followup_ids"] == [125]
