...
root=pathlib.Path.cwd();p=root/'validation/current-mixed-20260913-01'
...
with (p/'launch.json').open('x') as f:json.dump({'started_unix':time.time(),'diagnostic_only':False,'full_acceptance':False},f)
PY
set +e
bash tools/run-joint-flight.sh \
 ...
 2>&1 | tee validation/current-mixed-20260913-01/run.stdout.log
formal_exit=${PIPESTATUS[0]}
printf '%s\n' "$formal_exit" > validation/current-mixed-20260913-01/exit-code.txt
exit "$formal_exit"
