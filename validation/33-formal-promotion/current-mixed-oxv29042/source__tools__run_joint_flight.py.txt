...
def run(args):
    ...
    if getattr(args, 'early_work_timing', False):
        # Same gate as main(): programmatic run(args) must not bypass it.  This
        # raises before check_isolation, any temporary directory, or any child.
        if args.task_profile not in (PV_PROFILE, MIXED_PROFILE):
            raise ValueError('--early-work-timing is only allowed for PV/MIXED task profiles')
        if (os.environ.get('WKSIM_JOINT_CPU_TIMING') != '1'
    ...
    if getattr(args, 'manager_gc_freeze', False):
        sources += ['tools/manager_gc_candidate.py']
    if getattr(args, 'early_work_timing', False):
        sources += ['tools/early_manager_work_probe.py']
    ...
    early_probe = None
    if getattr(args, 'early_work_timing', False):
        from early_manager_work_probe import EarlyManagerWorkProbe
        early_probe = EarlyManagerWorkProbe()
    manager_gc = None
    if getattr(args, 'manager_gc_freeze', False):
        from manager_gc_candidate import ManagerGCFreeze
        manager_gc = ManagerGCFreeze(
            enabled=True,
            source_sha256=result['source_sha256'].get('tools/manager_gc_candidate.py'))
    try:
        if manager_gc is not None:
            manager_gc.arm()
        ...
        with ExitStack() as resources:
            ...
            pv_ready_paths = {leg:{stack:live/stack/f'pv-ready-{leg}.json' for stack in workers}
                              for leg in (1, 2)}
            if manager_gc is not None:
                manager_gc.prepare(monotonic_ns=time.monotonic_ns(), clock_tick=clock.tick)
            physics.connect()
            ...
            def advance():
                ...
                if rate is not None and clock.tick%4 == 0 and clock.synchronized and clock.phase == 'running':
                    if rate.anchor is None:
                        ...
                        if early_probe is not None:
                            early_probe.set_window(anchor_wall_ns=rate.anchor['wall_ns'],
                                                   epoch=clock.epoch,
                                                   segment=rate.segment_id)
                    if early_probe is not None:
                        early_probe.wrap('rate_begin_group', lambda: clock.tick, lambda: clock.epoch,
                                         lambda: rate.segment_id, rate.begin_group, clock.tick, physics_health)
                    else:
                ...
                if early_probe is not None:
                    states = early_probe.wrap('physics_advance', lambda: clock.tick, lambda: clock.epoch,
                                              lambda: rate.segment_id, physics.advance)
                    early_probe.wrap('clock_publication_log', lambda: clock.tick, lambda: clock.epoch,
                                     lambda: rate.segment_id,
                                     lambda: (publisher.publish(clock),
            ...
            while clock.tick < MAX_TICKS:
                if early_probe is not None:
                    early_probe.wrap('manager_health', lambda: clock.tick, lambda: clock.epoch,
                                     lambda: rate.segment_id, health)
                else:
                    health()
                ...
                if clock.tick==5000 and not candidate:
                    record_native_maps('running')
                early_region = (early_probe.begin('post_advance_readiness_summary', lambda: clock.tick,
                                           lambda: clock.epoch, lambda: rate.segment_id)
                                if early_probe is not None else None)
                try:
                    ...
                    if early_region is not None:
                        early_region.end(outcome='error')
                    raise
                if early_region is not None:
                    early_region.end()
        ...
        try:
            ...
            # Native cleanup and every ExitStack callback have already run,
            # so the graph we froze is released even if cleanup raised.
            if manager_gc is not None:
                try:
                    manager_gc.restore()
                except BaseException as error:
                    result['manager_gc_candidate_error'] = repr(error)
                result['manager_gc_candidate'] = manager_gc.report()
                try:
                    save(live/'manager-gc-candidate.json', result['manager_gc_candidate'])
                except BaseException as error:
                    result['manager_gc_candidate_write_error'] = repr(error)
            if early_probe is not None:
                try:
                    result['early_manager_work'] = early_probe.report(
                        source_sha256=result['source_sha256'].get('tools/early_manager_work_probe.py'),
                        identity=dict(run_id=result['run_id'], scene_epoch=clock.epoch,
                                      segment=(rate.segment_id if rate is not None else None)))
                    save(live/'early-manager-work.json', result['early_manager_work'])
                except BaseException as error:
                    result['early_manager_work_error'] = repr(error)
...
def main(argv=None):
    ...
    if args.role == 'run':
        ...
        if args.manager_gc_freeze and not (pv or mixed):
            parser.error('--manager-gc-freeze is only allowed for PV/MIXED task profiles')
        if args.early_work_timing:
            if not (pv or mixed):
                parser.error('--early-work-timing is only allowed for PV/MIXED task profiles')
            if (os.environ.get('WKSIM_JOINT_CPU_TIMING') != '1'
                    or os.environ.get('WKSIM_JOINT_RATE_TIMING_PROBE') != '1'):
