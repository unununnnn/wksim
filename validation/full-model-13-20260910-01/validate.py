...
def main():
    contract = json.loads((ROOT / 'docs/plan/full-contracts/model-13.json').read_text(encoding="utf-8"))
    ...
    ledger_row = next(row for row in ledger["entries"] if row.get("id") == 'MODEL-13')
    followup_row = next(row for row in followups["entries"] if row.get("id") == 'MODEL-13')
    assert contract["issue"] == 147
    assert contract["source"]["line"] == 58
    assert contract["full_complete"] is False
    assert ledger_row["gap"] is True
    assert followup_row["followup_ids"] == [147]
    ...
    text0 = (ROOT / 'Simulator/wksim_core/px4_mavlink.py').read_text(encoding="utf-8")
    assert 'HIL' in text0, 'HIL'
    scope = (ROOT / "docs/plan/full-scope-expansion.md").read_text(encoding="utf-8")
    assert '| MulticopterNOpx4 |' in scope

    hashes = json.loads((CASE / "hashes.json").read_text(encoding="utf-8"))["inputs"]
    for relative, expected in hashes.items():
        assert sha256(ROOT / relative) == expected, relative
    print(json.dumps({"status": "pass", "assertions": 15, "skipped": 0,
