...
def main():
    contract = json.loads((ROOT / 'docs/plan/full-contracts/model-06.json').read_text(encoding="utf-8"))
    ...
    ledger_row = next(row for row in ledger["entries"] if row.get("id") == 'MODEL-06')
    followup_row = next(row for row in followups["entries"] if row.get("id") == 'MODEL-06')
    assert contract["issue"] == 139
    assert contract["source"]["line"] == 51
    assert contract["full_complete"] is False
    assert ledger_row["gap"] is True
    assert followup_row["followup_ids"] == [139]
    assert len(contract["atoms"]) == 3
    assert len({atom["id"] for atom in contract["atoms"]}) == 3
    ...
    print(json.dumps({"status": "pass", "assertions": 17, "skipped": 0,
                      "atoms": 3, "full_complete": False}, sort_keys=True))
