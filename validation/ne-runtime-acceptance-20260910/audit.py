"""Independently audit one sealed NE flight without touching its evidence."""
import json
...
if len(sys.argv) > 3:
    ...
    label += '-' + revision
run_id = 'ne-' + stack + '-acceptance-20260910-' + attempt
...
command = ['/usr/bin/python3', '-B', '-c',
    "import sys,runpy;sys.path.insert(0,'/root/wksim-attitude-audit-deps-g_2y8olg');runpy.run_path('tools/audit_ne_flight.py',run_name='__main__')",
