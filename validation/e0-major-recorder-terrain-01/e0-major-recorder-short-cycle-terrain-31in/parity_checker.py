...
def check_parity(record_jsonl_path, ref_target, csv_input_path):
    ...
    try:
        start_obj = json.loads(record_lines[0])
    except Exception as err:
    ...
    try:
        end_obj = json.loads(record_lines[502])
    except Exception as err:
    ...
    for k in range(501):
        line_idx = k + 1
        try:
            obj = json.loads(record_lines[line_idx])
        except Exception as err:
    ...
    if is_ref_so:
        ...
        try:
            ref_start = json.loads(ref_lines[0])
        except Exception as err:
        ...
        try:
            ref_end = json.loads(ref_lines[502])
        except Exception as err:
        ...
        for k in range(501):
            line_idx = k + 1
            try:
                ref_obj = json.loads(ref_lines[line_idx])
            except Exception as err:
