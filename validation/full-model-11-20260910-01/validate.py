...
def main():
    contract = json.loads((ROOT / 'docs/plan/full-contracts/model-11.json').read_text(encoding="utf-8"))
    ...
    ledger_row = next(row for row in ledger["entries"] if row.get("id") == 'MODEL-11')
    followup_row = next(row for row in followups["entries"] if row.get("id") == 'MODEL-11')
    assert contract["issue"] == 145
    assert contract["source"]["line"] == 56
    assert contract["full_complete"] is False
    assert ledger_row["gap"] is True
    assert followup_row["followup_ids"] == [145]
