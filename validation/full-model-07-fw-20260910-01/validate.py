...
def main():
    contract = json.loads((ROOT / 'docs/plan/full-contracts/model-07-fw.json').read_text(encoding="utf-8"))
    ...
    ledger_row = next(row for row in ledger["entries"] if row.get("id") == 'MODEL-07')
    followup_row = next(row for row in followups["entries"] if row.get("id") == 'MODEL-07')
    assert contract["issue"] == 140
    assert contract["source"]["line"] == 52
    assert contract["full_complete"] is False
    assert ledger_row["gap"] is True
    assert followup_row["followup_ids"] == [140, 141]
    ...
    assert 'motor_count' in text0, 'motor_count'
    scope = (ROOT / "docs/plan/full-scope-expansion.md").read_text(encoding="utf-8")
    assert '| 固定翼、复合翼 |' in scope
    assert 'airfoil' not in (ROOT / 'Simulator/wksim_core/model_parameters.py').read_text(encoding='utf-8').lower()
