...
struct Context {
    ...
    unsigned long long start = 4000, end = 6000;
    bool fresh[2] = {};
...
{
    if (instance > 1 || context().tick >= context().start) fail("sensor_recreated_after_plan_start");
    auto &c = context();
...
template <typename Data> inline bool sample(unsigned instance, const Data &d, uint64_t hal_us)
{
    ...
    if (d.timestamp_ms == 0 && !c.warmed[instance] && c.tick < c.start) {
        c.warmed[instance] = true;
        fprintf(trace(), "WARMUP\t%llu\t%u\n", c.tick, instance); flush();
        return false;
    }
    if (d.timestamp_ms > c.tick ||
