...
def main():
    contract = json.loads((ROOT / 'docs/plan/full-contracts/model-16-exp2.json').read_text(encoding="utf-8"))
    ...
    assert sorted({atom["status"] for atom in contract["atoms"]}) == ['blocked']
    ...
    assert 'Exp2_MaxModelTemp' not in (ROOT / 'tools/probe_model_reference_readiness.py').read_text(encoding='utf-8')
