...
def main():
    contract = json.loads((ROOT / 'docs/plan/full-contracts/sim-11.json').read_text(encoding="utf-8"))
    ...
    ledger_row = next(row for row in ledger["entries"] if row.get("id") == 'SIM-11')
    followup_row = next(row for row in followups["entries"] if row.get("id") == 'SIM-11')
    assert contract["issue"] == 127
    assert contract["source"]["line"] == 23
    assert contract["full_complete"] is False
    assert ledger_row["gap"] is True
    assert followup_row["followup_ids"] == [127]
    ...
    text2 = (ROOT / 'Simulator/wksim_runtime/scene_clock.py').read_text(encoding="utf-8")
    assert 'class SceneClock' in text2, 'class SceneClock'
    scope = (ROOT / "docs/plan/full-scope-expansion.md").read_text(encoding="utf-8")
    assert '| 10 | PX4_SIH_SITL |' in scope
