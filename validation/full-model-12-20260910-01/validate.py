...
def main():
    contract = json.loads((ROOT / 'docs/plan/full-contracts/model-12.json').read_text(encoding="utf-8"))
    ...
    ledger_row = next(row for row in ledger["entries"] if row.get("id") == 'MODEL-12')
    followup_row = next(row for row in followups["entries"] if row.get("id") == 'MODEL-12')
    assert contract["issue"] == 146
    assert contract["source"]["line"] == 57
    assert contract["full_complete"] is False
    assert ledger_row["gap"] is True
    assert followup_row["followup_ids"] == [146]
    ...
    assert '| MulticopterNoCtrl |' in scope
    assert (ROOT / 'Simulator/wksim_core/model.cpp').is_file()
