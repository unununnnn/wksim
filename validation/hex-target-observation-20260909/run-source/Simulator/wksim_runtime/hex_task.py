...
AP_OBSERVATION_IDS = (87, 85, 245)  # Global/local target receipt and landed state.
...
class HexTask(Task):
    ...
    def request_observation_interval(self, message_id):
        """Only SET_MESSAGE_INTERVAL; this socket cannot issue motion commands."""
        if self.flight_stack != 'arducopter' or type(message_id) is not int or message_id not in AP_OBSERVATION_IDS:
            raise ValueError('Unsupported AP observation message')
        identity, peer = (self.epoch, self.native_generation), self.peer
        deadline = time.monotonic()+self.budget['parameter_read_timeout_s']
        def check():
            if (peer is None or self.peer != peer or self.target_system != 241 or not self.ground_current()
                    or (self.epoch, self.native_generation) != identity):
                raise RuntimeError('AP observation lost grounded native identity')
            if time.monotonic() >= deadline:
                raise TimeoutError('AP observation acknowledgement deadline')
        check()
        start = len(self.mav_messages)
        request = self.dialect.MAVLink_command_long_message(241, 1, 511, 0, message_id, 100000, 0, 0, 0, 0, 0)
        raw = request.pack(self.encoder)
        self.record('observation_interval_tx', message_id=message_id, interval_us=100000,
                    datagram_hex=raw.hex(), peer=peer, physical_cursor=self.cursor())
        if self.channel.sendto(raw, peer) != len(raw):
            raise RuntimeError('Incomplete observation request')
        def acknowledged():
            check()
            for row in self.mav_messages[start:]:
                m = row['message']
                if (row['source_system'] != 241 or row['source_component'] != 1 or row['peer'] != peer
                        or m.get('mavpackettype') != 'COMMAND_ACK' or m.get('command') != 511
                        or m.get('target_system') != 245 or m.get('target_component') != 190):
                    continue
                if m['result'] == 5:  # IN_PROGRESS is not acceptance.
                    continue
                self.record('observation_interval_ack', message_id=message_id, packet_hex=row['packet_hex'],
                            result=m['result'], physical_cursor=self.cursor())
                if m['result'] != 0:
                    raise RuntimeError('AP observation request rejected: '+str(m['result']))
                return True
            return False
        self.wait('observation_interval_'+str(message_id), acknowledged, self.budget['parameter_read_timeout_s'])

    def configure_target_observation(self):
        if self.flight_stack == 'arducopter':
            for message_id in AP_OBSERVATION_IDS:
                self.request_observation_interval(message_id)

    def wait_native_landed(self):
        if self.flight_stack != 'arducopter':
            return
        start, identity, peer = len(self.mav_messages), (self.epoch, self.native_generation), self.peer
        deadline = time.monotonic()+self.budget['parameter_read_timeout_s']
        def landed():
            if (not self.ground_current() or self.peer != peer or
                    (self.epoch, self.native_generation) != identity):
                raise RuntimeError('AP landed observation lost grounded native identity')
            if time.monotonic() >= deadline:
                raise TimeoutError('AP fresh landed observation deadline')
            return any(r['source_system'] == 241 and r['source_component'] == 1 and r['peer'] == peer
                       and r['message'].get('mavpackettype') == 'EXTENDED_SYS_STATE'
                       and r['message'].get('landed_state') == 1 for r in self.mav_messages[start:])
        self.wait('native_landed_observed', landed, self.budget['parameter_read_timeout_s'])
    ...
    def execute(self):
        ...
        self.configure_target_observation()
