...
root=pathlib.Path.cwd();p=root/'validation/formal-pv-settle-20260913-01'
...
bash tools/run-joint-flight.sh \
 --task-profile full_xyz_pv_yaw_v1 --async-model-evidence --manager-gc-freeze \
 ...
 2>&1 | tee validation/formal-pv-settle-20260913-01/run.stdout.log
formal_exit=${PIPESTATUS[0]}
printf '%s\n' "$formal_exit" > validation/formal-pv-settle-20260913-01/exit-code.txt
exit "$formal_exit"
