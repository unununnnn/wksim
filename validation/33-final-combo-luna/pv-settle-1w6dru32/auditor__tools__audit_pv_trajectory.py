...
def expected_public_error(event, event_row, message, data, *, stack, run_id, control_epoch, first_request_ns):
    """Only same-session bootstrap ACKs or independently corroborated status repeats."""
    from audit_mixed_control import identical_status_duplicate
    return (message['message_type'] == 2 and event.get('run_id') == run_id
            and event.get('control_epoch') == control_epoch
            and (rejected_bootstrap_ack(event, stamp(message), first_request_ns)
                 or identical_status_duplicate(event, event_row, data, stack)))
...
def task_evidence(root, result, data, *, recorder_name='wksim_joint_flight_clock', task_root=None,
    ...
    for stack, uid in STACKS:
        ...
        for row, message in data[base+'text_info']:
            ...
            require(message['message_type'] < 2 or expected_public_error(
                    event, row, message, data, stack=stack, run_id=result['run_id'],
                    control_epoch=task['control_epoch'], first_request_ns=first_request_ns),
...
def audit(root, *, task_profile=None):
    ...
    return dict(status='pass', task_profile=selected, run_id=result['run_id'], scene_epoch=result['scene_epoch'],
                ...
                rejected_unsolicited_bootstrap_acks={stack: [json.loads(m['message']) for _, m in
                    raw[f'/uav{uid}/prometheus/text_info'] if m['message_type'] == 2
                    and json.loads(m['message']).get('reason') == 'unmatched_ack'] for stack, uid in STACKS},
                corroborated_duplicate_status_events={stack: [json.loads(m['message']) for _, m in
                    raw[f'/uav{uid}/prometheus/text_info'] if m['message_type'] == 2
                    and json.loads(m['message']).get('reason') == 'duplicate_source'] for stack, uid in STACKS},
