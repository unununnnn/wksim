...
class PVTask(Task):
    ...
    def execute(self):
        ...
        at_waypoint = lambda: (self.fresh() and math.dist(self.state.position, [2, 3, 3]) <= .5
                              and math.hypot(*self.state.velocity) <= .5
                              and abs((self.state.attitude[2]+math.pi) % (2*math.pi)-math.pi) <= .15)
        # Native state is an estimate: settle below the physical speed limit
        # before opening the unchanged two-second, 0.5 m/s audited dwell.
        self.wait('waypoint_reached', lambda: at_waypoint() and math.hypot(*self.state.velocity) <= .4)
