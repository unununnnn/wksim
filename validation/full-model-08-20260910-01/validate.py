...
def main():
    contract = json.loads((ROOT / 'docs/plan/full-contracts/model-08.json').read_text(encoding="utf-8"))
    ...
    ledger_row = next(row for row in ledger["entries"] if row.get("id") == 'MODEL-08')
    followup_row = next(row for row in followups["entries"] if row.get("id") == 'MODEL-08')
    assert contract["issue"] == 142
    assert contract["source"]["line"] == 53
    assert contract["full_complete"] is False
    assert ledger_row["gap"] is True
    assert followup_row["followup_ids"] == [142]
