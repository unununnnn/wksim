...
work=$(mktemp -d /root/wksim-px4-land-XXXXXX)
printf 'PX4 land cadence candidate: %s\n' "$work"
mkdir "$work/src"
tar -C "$baseline" --exclude=./build -cf - . | tar -C "$work/src" -xf -
git -C "$work/src" apply --check "$repo/patches/px4/0004-land-observation-cadence.patch"
git -C "$work/src" apply "$repo/patches/px4/0004-land-observation-cadence.patch"
...
python3 -B "$repo/tools/px4_land_candidate.py" seal "$work"
