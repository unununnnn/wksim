...
static bool monotonic_ns(int clock_id, uint64_t *out)
{
    ...
    if (clock_gettime(clock_id, &now) != 0) {
        fatal("clock_gettime(%d) failed: %s", clock_id, strerror(errno));
        return false;
    }
                              ...
                              uint64_t observed, uint64_t expected)
{
    ...
    printf("  \"failure\": {\"reason\": \"%s\", \"observed\": %" PRIu64
           ", \"expected\": %" PRIu64 ", \"errno\": \"%s\"},\n",
           reason, observed, expected, strerror(errno));
...
static void report_json(const struct probe_state *state, const perf_parse_summary *summary,
                        ...
                        bool cleanup_ok, bool timing_ok, const unsigned char *cpu_seen)
{
    bool ok = summary->ok && cleanup_ok && timing_ok;
    printf("{\n");
    printf("  \"tool\": \"perf_self_capability\",\n");
    printf("  \"version\": \"v2\",\n");
    printf("  \"classification\": \"capability_probe_only\",\n");
...
int main(void)
{
    ...
    if (state.pid <= 0 || state.tid <= 0) {
        ...
        fprintf(stderr, "%s\n", g_fatal);
        emit_failure_json(&state, "identity_unavailable", 0, 0);
        return 2;
    }

    long page_long = sysconf(_SC_PAGESIZE);
    if (page_long <= 0) {
        emit_failure_json(&state, "sysconf_failed", 0, 0);
        return 2;
    }
    uint64_t page_size = (uint64_t)page_long;
    if (!is_positive_power_of_two(page_size)) {
        emit_failure_json(&state, "page_size_not_power_of_two", page_size, 0);
        return 2;
    }
    ...
    if (state.fd < 0) {
        ...
        snprintf(g_fatal, sizeof g_fatal,
                 ...
                 strerror(errno), errno, errno_class_name(klass));
        fprintf(stderr, "%s\n", g_fatal);
    ...
    if (mapping == MAP_FAILED) {
        emit_failure_json(&state, "mmap_failed", state.map_size, 0);
        close(state.fd);
        return 2;
    }
    state.mapped = true;
    ...
    if (failure != 0 && failure_name != NULL) {
         ...
         * reason, not the parser's "ok" and not a generic bad_argument */
        summary.status = PERF_PARSE_PROBE_FAILURE;
        snprintf(summary.first_problem, sizeof summary.first_problem, "%s", failure_name);
        if (g_fatal[0] == '\0') {
            snprintf(g_fatal, sizeof g_fatal, "%s", failure_name);
        }
    }
    report_json(&state, &summary, elapsed_ns, head, tail, cleanup_ok, timing_ok, cpu_seen);
