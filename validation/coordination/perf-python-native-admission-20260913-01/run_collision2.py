...
assert not (root / 'collision2-receipt.json').exists()
...
for distro in ('Ubuntu-22.04', 'RflySim-20.04'):
    ...
    checks.append(check)
(root / 'collision2-prechecks.json').write_text(json.dumps(checks, indent=2) + '\n')
...
before={'fds':len(list(pathlib.Path('/proc/self/fd').iterdir())),'threads':len(list(pathlib.Path('/proc/self/task').iterdir()))}
cap.start()
try:
...
assert 'stop: create raw' in failure and 'errno 17' in failure
assert not (out/'switch.meta.json').exists()
assert (out/'switch.raw').read_bytes()==b'preserve original'
after={'fds':len(list(pathlib.Path('/proc/self/fd').iterdir())),'threads':len(list(pathlib.Path('/proc/self/task').iterdir()))}
assert before==after,(before,after)
print(json.dumps({'failure':failure,'retained':cap.owns_handle,'stop_completed':cap.stop_completed,'resources_before':before,'resources_after':after,'original_preserved':True,'metadata_created':False,'counter_inspected':False}))
"""
...
(root / 'collision2-receipt.json').write_text(json.dumps(receipt, indent=2) + '\n')
