...
export WKSIM_JOINT_RATE_TIMING_PROBE=1
python3 -B - <<'PY'
import hashlib,json,pathlib,time
root=pathlib.Path.cwd();p=root/'validation/last-callbacks-run-20260913-01'
...
import os
if os.environ.get('WKSIM_JOINT_RATE_TIMING_PROBE')!='1' or os.environ.get('WKSIM_JOINT_CPU_TIMING') is not None:
 raise RuntimeError('parent-only diagnostic environment differs')
...
bash tools/run-joint-flight.sh \
 --owned-scheduling-snapshot --task-profile xy_velocity_z_position_yaw_v1 --async-model-evidence --manager-gc-freeze \
 ...
 2>&1 | tee validation/last-callbacks-run-20260913-01/run.stdout.log
formal_exit=${PIPESTATUS[0]}
printf '%s\n' "$formal_exit" > validation/last-callbacks-run-20260913-01/exit-code.txt
exit "$formal_exit"
