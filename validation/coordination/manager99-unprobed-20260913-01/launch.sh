...
root=pathlib.Path.cwd();p=root/'validation/manager99-unprobed-20260913-01'
r=json.loads((p/'pre-run-identity.json').read_text())
boot=pathlib.Path('/proc/sys/kernel/random/boot_id').read_text().strip()
for distro in ('Ubuntu-22.04','RflySim-20.04'):
 check=json.loads((p/('precheck-'+distro+'.json')).read_text())
 if check['distro']!=distro or check['boot_id']!=boot or check['found'] or not -1<=time.time()-check['checked_unix']<=60:raise RuntimeError('refresh both native prechecks before launch')
...
import os
if os.environ.get('WKSIM_JOINT_RATE_TIMING_PROBE') is not None or os.environ.get('WKSIM_JOINT_CPU_TIMING') is not None:
 raise RuntimeError('unprobed environment differs')
with (p/'launch.json').open('x') as f:json.dump({'started_unix':time.time(),'unprobed_trial':True,'full_acceptance':False},f)
PY
set +e
bash tools/run-joint-flight.sh \
 --task-profile xy_velocity_z_position_yaw_v1 --async-model-evidence --manager-gc-freeze \
 ...
 2>&1 | tee validation/manager99-unprobed-20260913-01/run.stdout.log
formal_exit=${PIPESTATUS[0]}
printf '%s\n' "$formal_exit" > validation/manager99-unprobed-20260913-01/exit-code.txt
exit "$formal_exit"
