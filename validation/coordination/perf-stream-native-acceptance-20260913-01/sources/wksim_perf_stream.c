    ...
    if (count == 1) {
        ...
        if (saved_errno != 0) {
            snprintf(reason, sizeof reason, "%s (errno %d: %s)",
                     wksim_err_name[code], saved_errno, strerror(saved_errno));
        } else {
            snprintf(reason, sizeof reason, "%s", wksim_err_name[code]);
        }
        snprintf(message, sizeof message, "%s: %s", where ? where : "failure", reason);
...
static void settle_errors(struct wksim_perf_stream *stream)
{
    ...
    if (stream->error_count > 0) {
        ...
        if (primary->saved_errno != 0) {
            snprintf(message, sizeof message, "%s (errno %d: %s)", primary->text,
                     primary->saved_errno, strerror(primary->saved_errno));
        } else {
            snprintf(message, sizeof message, "%s", primary->text);
        }
        set_last_error(message);
    }
    pthread_mutex_unlock(&stream->error_lock);
...
static int join_reader(struct wksim_perf_stream *stream, bool mark_joined)
{
    ...
    if (rc == ESRCH) {
        /* The thread is already gone: resources are safe to release, but the anomaly is
         * recorded rather than hidden. */
        stream->reader_running = false;
        if (mark_joined) {
            stream->reader_joined = true;
        }
        record_error(stream, WKSIM_ERR_JOIN, rc, "join: reader already gone (ESRCH)");
        return 0;
    }
    if (rc != 0) {
        stream->join_failed = true;
        stream->reader_running = true;      /* still owns the handle and its resources */
        return rc;
    }
    stream->reader_running = false;
    if (mark_joined) {
 ...
 * join retains every resource instead of freeing memory the collector may still be
 * using; start cannot hand the handle back on this path, so it is deliberately leaked
 * (recorded as WKSIM_ERR_JOIN), matching the stop-side ownership refusal. */
static int start_abort(struct wksim_perf_stream *stream, int saved)
{
    ...
    if (join_rc != 0) {
        record_error(stream, WKSIM_ERR_JOIN, join_rc,
                     "start: reader join failed, resources retained");
    }
...
int wksim_perf_start(void **handle)
{
    ...
    *handle = NULL;
     ...
     * thread. On every failure path the handle is freed wholesale, so they are never
     * destroyed and never used after free. */
    ...
    if (stream->storage == MAP_FAILED) {
        int saved = errno;
        stream->storage = NULL;
        record_error(stream, WKSIM_ERR_STORAGE_MAP, saved, "start: storage mmap");
        release_handle(stream);
        free(stream);
        errno = saved;
        return -1;
