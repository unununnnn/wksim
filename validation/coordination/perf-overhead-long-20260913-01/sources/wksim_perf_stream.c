...
#define WKSIM_PERF_FORMAT_LOST (1ull << 4) /* Linux 6.0+ UAPI; runtime checked */
...
struct wksim_perf_stream {
    ...
    bool enable_ok;
    bool lost_read_ok;
    uint64_t kernel_lost_count;
    ssize_t lost_read_bytes;
    int lost_read_errno;
...
int wksim_perf_stop(void **handle, const char *raw_path, const char *meta_path)
{
    ...
    if (disable_ok) {
        uint64_t counters[2] = {0, 0}; /* non-group value followed by PERF_FORMAT_LOST */
        for (int attempt = 0; attempt < 8; ++attempt) {
            stream->lost_read_bytes = read(stream->fd, counters, sizeof counters);
            stream->lost_read_errno = stream->lost_read_bytes < 0 ? errno : 0;
            if (stream->lost_read_bytes >= 0 || stream->lost_read_errno != EINTR) break;
        }
        stream->lost_read_ok = stream->lost_read_bytes == (ssize_t)sizeof counters;
        if (stream->lost_read_ok) {
            stream->kernel_lost_count = counters[1];
            if (stream->kernel_lost_count != 0)
                record_error(stream, WKSIM_ERR_CAPTURE_INCOMPLETE, 0,
                             "stop: kernel event loss counter is nonzero");
        } else {
            record_error(stream, WKSIM_ERR_CAPTURE_INCOMPLETE, stream->lost_read_errno,
                         "stop: PERF_FORMAT_LOST read unavailable or wrong size");
        }
    }
    ...
    if (meta_fd >= 0) {
        if (stream->lost_read_ok)
            snprintf(lost_count_text, sizeof lost_count_text, "%" PRIu64,
                     stream->kernel_lost_count);
        else
            snprintf(lost_count_text, sizeof lost_count_text, "null");
        ...
        written = snprintf(
            ...
            "  \"kernel_lost_read_ok\": %s,\n"
            "  \"kernel_lost_read_bytes\": %zd,\n"
            "  \"kernel_lost_read_errno\": %d,\n"
            "  \"kernel_lost_count\": %s,\n"
            "  \"losslessness_proven\": false,\n"
            "  \"losslessness_reason\": \"independent raw and kernel-counter validation "
            "is required; collector_complete alone is not a validation result\",\n"
            "  \"config\": {\"pid_argument\": 0, \"cpu_argument\": -1, \"inherit\": 0, "
            "\"exclude_kernel\": 1, \"context_switch\": 1, \"sample_id_all\": 1, "
            "\"read_format\": 16, \"sample_type\": [\"TID\", \"TIME\", "
            "\"CPU\"]},\n"
            ...
            stream->lost_read_ok ? "true" : "false", stream->lost_read_bytes,
            stream->lost_read_errno, lost_count_text,
