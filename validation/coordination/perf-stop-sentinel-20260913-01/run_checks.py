...
repo=root.parents[2]
ancestor=subprocess.run(['git','-C',str(repo),'merge-base','--is-ancestor','f333316e6efa6b299b4288a9d91fb2bccedfb9d6','HEAD'])
...
paths=[author/n for n in ['wksim_perf_stream.c','wksim_perf_stream.h','wksim_perf_stream_demo.c']]+[root/n for n in ['create_failure_case.c','join_failure_case.c','ring_copy_cases.c','policy_failure_case.c','cleanup_failure_case.c']]
...
linux="import base64,hashlib,json,os,pathlib,signal,subprocess,tempfile,time\nboot=pathlib.Path('/proc/sys/kernel/random/boot_id').read_text().strip()\nassert all(c['boot_id']==boot and 0<=time.time()-c['checked_unix']<=60 and not c['found'] for c in payload['checks'])\nroot=pathlib.Path(tempfile.mkdtemp(prefix='wksim-stream-native-',dir='/root'))\nfor name,raw in payload['sources'].items():(root/name).write_bytes(base64.b64decode(raw))\n(root/'capture').mkdir()\nout={'directory':str(root),'boot_id':boot,'commands':[],'source_sha256':{n:hashlib.sha256((root/n).read_bytes()).hexdigest() for n in payload['sources']}}\nflags=['cc','-std=c11','-O2','-Wall','-Wextra','-Werror','-pthread','-I.']\ncommands=[flags+['-o','demo','wksim_perf_stream_demo.c','wksim_perf_stream.c'],flags+['-Wl,--wrap=pthread_create','-o','create_failure','create_failure_case.c','wksim_perf_stream.c'],flags+['-Wl,--wrap=pthread_join','-o','join_failure','join_failure_case.c'],flags+['-o','ring_copy','ring_copy_cases.c'],flags+['-Wl,--wrap=pthread_getschedparam','-Wl,--wrap=ioctl','-o','policy_failure','policy_failure_case.c','wksim_perf_stream.c'],flags+['-Wl,--wrap=munmap','-o','cleanup_failure','cleanup_failure_case.c','wksim_perf_stream.c'],['./create_failure'],['./join_failure'],['./ring_copy'],['./policy_failure'],['./cleanup_failure',str(root/'capture')]]\nfor argv in commands:\n p=subprocess.Popen(argv,cwd=root,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True,start_new_session=True)\n row={'argv':argv,'pid':p.pid}\n try:stdout,stderr=p.communicate(timeout=20)\n except subprocess.TimeoutExpired:\n  os.killpg(p.pid,signal.SIGKILL);stdout,stderr=p.communicate();row['timed_out']=True\n row.update(exit_code=p.returncode,stdout=stdout,stderr=stderr)\n try:os.killpg(p.pid,0);row['pgid_empty']=False\n except ProcessLookupError:row['pgid_empty']=True\n if argv[0]=='./cleanup_failure':\n  try:\n   meta=json.loads((root/'capture/cleanup.meta.json').read_text())\n   row['metadata_reflects_cleanup_failure']=(meta['lifecycle']['munmap_ok'] is False and meta['collector_complete'] is False)\n  except Exception as e:row['metadata_reflects_cleanup_failure']=False;row['metadata_error']=str(e)\n out['commands'].append(row)\n if p.returncode or not row['pgid_empty'] or row.get('metadata_reflects_cleanup_failure') is False:break\nout['capture_files']={p.name:base64.b64encode(p.read_bytes()).decode() for p in (root/'capture').iterdir() if p.is_file()}\nout['boot_after']=pathlib.Path('/proc/sys/kernel/random/boot_id').read_text().strip()\nout['binary_sha256']={n:hashlib.sha256((root/n).read_bytes()).hexdigest() for n in ['demo','create_failure','join_failure','ring_copy','policy_failure'] if (root/n).exists()}\n(root/'receipt.json').write_text(json.dumps(out,indent=2));print(json.dumps(out))\n"
...
out['source_checkout']=str(repo)
out['source_head']=source_head
out['architecture_ancestor_exit_code']=0
out['work_class']='new-development: independent perf diagnostic module'
out['architecture_acceptance']=False
