...
def validate_stop_sentinel(meta, bounds, pairs):
    fields = ('sentinel_completed', 'sentinel_before_ns', 'sentinel_after_ns',
              'drain_confirmed_free_bytes', 'verifiable_window_end_ns')
    if not any(name in meta for name in fields):
        return None  # Legacy raw inspection does not acquire a completeness claim.
    if not all(name in meta for name in fields):
        raise ContractError('sentinel_fields_missing')
    if meta['sentinel_completed'] is not True:
        raise ContractError('sentinel_not_completed')
    for name in fields[1:]:
        if not is_int_in_range(meta[name], 0):
            raise ContractError('sentinel_field_not_integer', name)
    before, after = meta['sentinel_before_ns'], meta['sentinel_after_ns']
    if not (bounds['inner_first_ns'] <= before < after <= bounds['inner_last_ns']):
        raise ContractError('sentinel_bounds_invalid')
    if meta['verifiable_window_end_ns'] != before:
        raise ContractError('sentinel_cutoff_mismatch')
    if not (RING_PRESSURE_GUARD_BYTES <= meta['drain_confirmed_free_bytes']
            <= meta['ring_bytes']):
        raise ContractError('sentinel_drain_headroom_invalid')
    observed = [pair['sequence'] for pair in pairs
                if before <= pair['out_time_ns'] < pair['in_time_ns'] <= after]
    if not observed:
        raise ContractError('sentinel_pair_missing')
    return dict(verified_from_ns=bounds['inner_first_ns'], verified_until_ns=before,
                sentinel_before_ns=before, sentinel_after_ns=after,
                observed_pair_sequences=observed,
                scope='only windows ending before the stop check; final tail is not certified')
...
def main(argv=None):
    ...
    try:
        ...
        window_completeness = validate_stop_sentinel(meta, bounds, pairs)
        windows = None
        windows_sha = None
        intersections = None
        if args.windows:
            windows, windows_sha = load_windows(args.windows, meta, bounds)
            if window_completeness is not None and any(
                    w['end_ns'] > window_completeness['verified_until_ns'] for w in windows):
                raise ContractError('window_after_stop_check')
        ...
        report = dict(
            ...
            stream_completeness_proven=False,
            stream_completeness_reason=(
                'a verified stop check only certifies the earlier window range; the final '
                'tail after that check may have pending LOST and is not certified; legacy captures without a check '
                'remain unproven'),
            pairs=pairs,
            ...
            notes=[
                ...
                'stream_completeness_proven is false: this consumer cannot prove absence of '
                'loss from the retained bytes and the current metadata alone',
            ],
        )
