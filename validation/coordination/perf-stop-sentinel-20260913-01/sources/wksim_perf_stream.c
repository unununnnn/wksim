...
struct wksim_perf_stream {
    ...
    _Atomic bool checkpoint_requested;
    _Atomic bool checkpoint_ready;
    uint64_t drain_confirmed_free_bytes; /* reader publishes before checkpoint_ready */
    uint64_t sentinel_before_ns;
    uint64_t sentinel_after_ns;
    bool sentinel_completed;
...
{
    snprintf(wksim_last_error, sizeof wksim_last_error, "%.*s",
             (int)sizeof wksim_last_error - 1, text);
}
...
static void *wksim_perf_reader(void *argument)
{
    ...
    while (!atomic_load_explicit(&stream->stop_requested, memory_order_acquire)) {
        ...
        if (atomic_load_explicit(&stream->checkpoint_requested, memory_order_acquire)
                && !atomic_load_explicit(&stream->checkpoint_ready, memory_order_relaxed)) {
            uint64_t head = __atomic_load_n(&stream->meta->data_head, __ATOMIC_ACQUIRE);
            uint64_t tail = __atomic_load_n(&stream->meta->data_tail, __ATOMIC_RELAXED);
            if (head >= tail && head - tail <= ring_bytes
                    && ring_bytes - (head - tail) >= WKSIM_PERF_PRESSURE_GUARD
                    && error_count_now(stream) == 0) {
                stream->drain_confirmed_free_bytes = ring_bytes - (head - tail);
                atomic_store_explicit(&stream->checkpoint_ready, true, memory_order_release);
            }
        }
...
int wksim_perf_start(void **handle)
{
    ...
    *handle = NULL;
    ...
    if (!reader_ready) {
        ...
        return start_abort(stream, saved, handle);
    }

    if (!reader_policy_ok) {
        /* Reject the ready policy result before resetting or enabling perf. */
        record_error(stream, WKSIM_ERR_THREAD_POLICY, 0,
                     "start: reader is not SCHED_OTHER");
        return start_abort(stream, EPERM, handle);
    }
...
/* Owner-only, after the caller closed its measured work interval. A successful
 * sleep is not a proof of a switch: the offline consumer must find the actual
 * out/in pair in these bounds. No pacing loop or existing thread policy changes. */
static bool sentinel_pause(struct wksim_perf_stream *stream, uint64_t deadline)
{
    struct timespec pause = {0, 1000000};
    while (nanosleep(&pause, &pause) != 0) {
        int saved = errno;
        if (saved != EINTR) {
            record_error(stream, WKSIM_ERR_CLOCK, saved, "sentinel: nanosleep");
            return false;
        }
        uint64_t now;
        if (!monotonic_ns(CLOCK_MONOTONIC, &now)) {
            record_error(stream, WKSIM_ERR_CLOCK, errno, "sentinel: interrupted sleep clock");
            return false;
        }
        if (now >= deadline) {
            record_error(stream, WKSIM_ERR_CLOCK, ETIMEDOUT, "sentinel: sleep deadline");
            return false;
        }
    }
    return true;
...
static bool stop_sentinel(struct wksim_perf_stream *stream)
{
    uint64_t now;
    if (!monotonic_ns(CLOCK_MONOTONIC, &now)) {
        record_error(stream, WKSIM_ERR_CLOCK, errno, "sentinel: initial clock");
        return false;
    }
    uint64_t deadline = now + 1000000000ull;
    atomic_store_explicit(&stream->checkpoint_requested, true, memory_order_release);
    while (!atomic_load_explicit(&stream->checkpoint_ready, memory_order_acquire)) {
        if (error_count_now(stream) != 0) return false;
        if (!monotonic_ns(CLOCK_MONOTONIC, &now)) {
            record_error(stream, WKSIM_ERR_CLOCK, errno, "sentinel: drain clock");
            return false;
        }
        if (now >= deadline) {
            record_error(stream, WKSIM_ERR_CAPTURE_INCOMPLETE, ETIMEDOUT,
                         "sentinel: reader drain deadline");
            return false;
        }
        if (!sentinel_pause(stream, deadline)) return false;
    }
    if (!monotonic_ns(CLOCK_MONOTONIC, &stream->sentinel_before_ns)) {
        record_error(stream, WKSIM_ERR_CLOCK, errno, "sentinel: before clock");
        return false;
    }
    if (!sentinel_pause(stream, deadline)) return false;
    if (!monotonic_ns(CLOCK_MONOTONIC, &stream->sentinel_after_ns)) {
        record_error(stream, WKSIM_ERR_CLOCK, errno, "sentinel: after clock");
        return false;
    }
    if (stream->sentinel_after_ns >= deadline || error_count_now(stream) != 0) {
        record_error(stream, WKSIM_ERR_CAPTURE_INCOMPLETE, ETIMEDOUT,
                     "sentinel: completion deadline or collector error");
        return false;
    }
    stream->sentinel_completed = true;
    return true;
}

int wksim_perf_stop(void **handle, const char *raw_path, const char *meta_path)
{
    struct wksim_perf_stream *stream = handle == NULL ? NULL : *handle;
    ...
    if (stream->enable_ok && stream->reader_running
            && !atomic_load_explicit(&stream->stop_requested, memory_order_acquire)
            && error_count_now(stream) == 0) {
        (void)stop_sentinel(stream); /* failures are retained, then normal cleanup proceeds */
    }
    ...
    if (meta_fd >= 0) {
        ...
        written = snprintf(
            ...
            "  \"sentinel_completed\": %s,\n"
            "  \"sentinel_before_ns\": %" PRIu64 ",\n"
            "  \"sentinel_after_ns\": %" PRIu64 ",\n"
            "  \"drain_confirmed_free_bytes\": %" PRIu64 ",\n"
            "  \"verifiable_window_end_ns\": %" PRIu64 ",\n"
            ...
            "capture; offline sentinel validation covers only earlier windows, not the final tail\",\n"
            "  \"config\": {\"pid_argument\": 0, \"cpu_argument\": -1, \"inherit\": 0, "
            "\"exclude_kernel\": 1, \"context_switch\": 1, \"sample_id_all\": 1, "
            "\"sample_type\": [\"TID\", \"TIME\", "
            "\"CPU\"]},\n"
            ...
            stream->sentinel_completed ? "true" : "false",
            stream->sentinel_before_ns, stream->sentinel_after_ns,
            stream->drain_confirmed_free_bytes, stream->sentinel_before_ns,
    ...
    destroy_locks(stream);
    free(stream);
    *handle = NULL; /* A non-NULL handle unambiguously means ownership was retained. */
    return complete ? 0 : -1;
}
