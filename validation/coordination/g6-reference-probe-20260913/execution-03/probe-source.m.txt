function probe_reference_first_step(model_dir, input_csv, out_dir)
...
if nargin < 3
    error('wk:probe:args', 'usage: probe_reference_first_step(model_dir, input_csv, out_dir)');
end
...
for name = {'wk_probe_log','wk_probe_listeners','wk_probe_regok','wk_probe_register','wk_probe_input','wk_probe_dropped'}
...
try
    ...
    assignin('base', 'wk_probe_register', @() local_register(targets));
...
function local_register(targets)
...
try
    evalin('base', 'clear wk_probe_log wk_probe_listeners wk_probe_regok wk_probe_register wk_probe_input wk_probe_tout wk_probe_yout wk_probe_dropped');
catch
end
try
    if bdIsLoaded(model)
