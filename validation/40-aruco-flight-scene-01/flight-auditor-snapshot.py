...
def audit(root):
    ...
    counts=Counter();results=[];origin=None;first_step=None;previous_step=-1
    for entry in report['frames']:
        ...
        require(image is not None and image.shape==(480,640,4) and np.all(image[:,:,3]==255),'Invalid raw RGBA')
        ...
        if phase=='occluded':
            ...
            require(error<=2 and max(translation,world_error,body_error)<=.035,
                f'Geometry gate failed at {step}: {error}px/{translation}m/{world_error}world/{body_error}body')
            ...
            if results and phase==results[-1]['phase']=='moving':
                if entry['now_step']>results[-1]['step']+300:
                    ...
                    require(target['velocity_world_ue_mps'] is not None and np.max(np.abs(
                        np.asarray(target['velocity_world_ue_mps'])-[0,.25,0]))<=.12,'World motion diagnostic gate failed')
    ...
    return dict(status='pass',scope='Real airborne case5 rendering/geometry/occlusion only; no camera-driven flight claim',
        counts=dict(counts),report_sha256=digest(root/'report.json'),auditor_sha256=digest(__file__),frames=results)
